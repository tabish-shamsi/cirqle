...
const emailOTP = async ({ emailType, email, resend }: EmailOTPProps) => {
  try {
    ...
    if (emailType === "change_password" && user.allowChangePassword)
    ...
    if (emailType === "change_email" && user.lastEmailChange) {
      ...
      if (lastChange > now) {
        return {
          error:
            "Email change temporarily unavailable. Please contact support.",
        };
      }

      const timeSinceLastChange = Date.now() - lastChange;
      const changedWithin30Days =
        timeSinceLastChange < EMAIL_CHANGE_RESET_WINDOW;
    ...
    const alreadySentOTP = await OTP.findOne({
      userId: user._id,
      type: emailType,
    });
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        html: await render(
          OtpActionEmail({ type: emailType, otp, name: user.name }),
        ),
      });
    } catch (error) {
      await OTP.deleteMany({ userId: user._id, type: emailType });
