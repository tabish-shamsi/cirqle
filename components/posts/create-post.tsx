...
type PreviewType = { url: string; type: string, status: "pending" | "success" | "error", id: string }
      ...
      files.map(async (file: File) => {
        ...
        if (mediaRes) {
          if (mediaRes.error) {
            ...
            setPreviews((prev) =>
              prev.map((p) =>
                p.id === id ? { ...p, status: "success", id: mediaRes?.fileId } : p
              )
            )
    ...
    <Card className="w-full gap-4">
      ...
      <CardContent>
        <div className="relative">
          ...
          />
          {contentError && <p className="text-sm text-destructive mt-1">{contentError}</p>}
                ...
                {previews.map((media) => (
                  <div key={media.id} className="relative rounded-xl overflow-hidden">
                    {media.type === "image" ? (
                      ...
                      <video src={media.url} className="h-40 w-full object-cover" />
                    )}
                    ...
                    <Button onClick={() => removeMedia(media.id)} variant="ghost" className="absolute top-2 right-2 rounded-full w-8 h-8"><X /></Button>
      ...
      <CardFooter className="flex flex-wrap justify-between">
        ...
        <Button disabled={isUploadingMedia || media.length >= 4} onClick={() => fileInputRef.current?.click()} variant="ghost">
        ...
        <Button onClick={handleUpload} disabled={isLoading && isUploadingMedia && loading} >{loading ? "Creating Post..." : "Create Post"}</Button>
      </CardFooter>
    </Card >
  );
}
